...
SET @gDatabaseName = LOWER('bits'); -- 'bits' is mandatory
SET @gDatabaseSimpleEntities = '"Entity","Language","Schema","Individual","Name"'; -- Entity & Language & Schema are mandatory
SET @gDatabaseComplexEntities = '"Membership","Whereabouts"'; -- Entities with fields other than the default fields, e.g. Membership
SET @gDatabaseSimpleLinkedEntities = '"Entity-Name","Language-Name","Individual-Name"'; -- e.g. Entity-Name
SET @gDatabaseComplexLinkedEntities = '';
SET @gDatabaseSimpleLinkedEntitiesSeparator = '-';
SET @gDatabaseLanguage = 'English'; -- 'English' is mandatory
...
CREATE PROCEDURE `SP_GET_FIELD_VALUE` (
	...
	BEGIN
		...
		START TRANSACTION;
			...
			IF(_andOrWhere != '') THEN
				SET _query = CONCAT("
					SELECT `" , _requiredFieldName, "` 
					FROM `", _tableEntityName, "` 
					WHERE `", _whereFieldName, "` ", _whereOperator, " '", _whereValue, "' 
					", _andOrWhere, " 
					`", _andOrWhereFieldName, "` ", _andOrWhereOperator, " '", _andOrWhereValue,  "' 
					INTO @REQUIRED_FIELD_VALUE
				");
			ELSE
				SET _query = CONCAT("
					SELECT `" , _requiredFieldName, "` 
				...
				");			
			END IF;
			SELECT _query AS SP_GET_FIELD_VALUE;
...
CREATE PROCEDURE `SP_ADD_TYPES_TO_SCHEMA` (
	...
	BEGIN
		SET @gDatabaseName = DATABASE_NAME;
...
--		===== WE ARE HERE ====		
		...
		CALL SP_ADD_KEYS_TO_TYPES_IN_SCHEMA(@gDatabaseName, @entityName, @entityType, @viewEntityName, @viewKindOfEntityName, @tableEntityName, @fieldPrimaryKeyEntityID, @schemaKeysID);
...
CREATE PROCEDURE `SP_INSERT_ARRAY_OF_VALUES` (
	...
	BEGIN
		...
		WHILE (LOCATE(@separator, @valueFieldEntityValueArray) > 0) DO
			...
			CALL `SP_INSERT_INTO_TABLE` (@gDatabaseName, @entityName, @valueFieldPrimaryKeyEntityID, @valueFieldForeignKeyParentID, @valueFieldEntityKey, @valueFieldEntityValue);
			SET @valueFieldPrimaryKeyEntityID = @valueFieldPrimaryKeyEntityID + 1;
			SET @valueFieldForeignKeyParentID = @valueFieldPrimaryKeyEntityID;
		END WHILE;
		SET FOREIGN_KEY_CHECKS = 1; -- on
